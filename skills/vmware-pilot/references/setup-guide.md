...
pip install vmware-pilot==1.10.0 -i https://pypi.tuna.tsinghua.edu.cn/simple
```
...
```json
{
  "mcpServers": {
    "vmware-pilot": {
      "command": "uvx",
      "args": ["--from", "vmware-pilot==1.10.0", "vmware-pilot-mcp"]
    }
  }
}
```
...
(`uv tool install vmware-pilot==1.10.0`) and launch `vmware-pilot mcp`, which touches the network
zero times.
