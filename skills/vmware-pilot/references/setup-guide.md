...
pip install vmware-pilot==1.11.1 -i https://pypi.tuna.tsinghua.edu.cn/simple
```
...
```json
{
  "mcpServers": {
    "vmware-pilot": {
      "command": "uvx",
      "args": ["--from", "vmware-pilot==1.11.1", "vmware-pilot-mcp"]
    }
  }
}
```
...
(`uv tool install vmware-pilot==1.11.1`) and launch `vmware-pilot mcp`, which touches the network
zero times.
